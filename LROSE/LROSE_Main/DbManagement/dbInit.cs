...
namespace LROSE_Main.DbManagement
{
    public partial class dbInit : Form
    {
        ...
        public SqlConnection getSqlConnection(string dbName)
        {
            ...
            string connStr = LROSRDbContext.GetEFConnctionString(dbName);
        ...
        public List<string> getAllDbName()
        {
            ...
            {
                return GetALLDbName_sqlLite();
            }
            else
            {
                return GetALLDbName_sqlServer();
            }

        }

        private List<string> GetALLDbName_sqlServer()
        {
        ...
        private List<string> GetALLDbName_sqlLite()
        {
            List<string> dbPathList = new List<string>();
            List<string> dbNameList = new List<string>();
            string xmlPath = GetDirectoryPath();
            dbPathList = Directory.GetFiles(xmlPath, "*.*", SearchOption.AllDirectories).ToList();
            if (dbPathList.Count != 0)
            {
                foreach (string dbPath in dbPathList)
                {
                    dbNameList.Add(dbPath.Replace(".\\LROSEDB\\","").Trim());
                }
            }
            return dbNameList;
        }
        ...
        public void changeConfig(string AppValue)
        {
            try
            {
                ...
                AppValue = LROSRDbContext.GetEFConnctionString(AppValue);
